...
class ProductAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        user_obj.save()
        product_post = Product.objects.create(user=user_obj, name='New Name', content='some_random_content whey')
    ...
    def test_get_list(self):
        data = {}
        url = api_reverse('api-products:product-list_create')
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(response.data)

    def test_product_item(self):
        data = {'name': 'some random name', 'content': 'some more content'}
        url = api_reverse('api-products:product-list_create')
    ...
    def test_update_item(self):
        ...
        product_post = Product.objects.first()
        url = product_post.get_api_url()
        data = {'name': 'some random name', 'content': 'some more content'}
        response = self.client.pproduct(url, data, format='json')
    ...
    def test_user_login(self):
        ...
        rsp = self.client.product(url, data)
        print(rsp.data)
