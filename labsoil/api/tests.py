...
class LabPostAPITestCase(APITestCase):

    def setUp(self):
        user_obj = User.objects.create(username='testcfeuser', email='test@test.com')
        user_obj.set_password('somerandopassword')
        user_obj.save()
        blog_post = Lab.objects.create(user=user_obj, title='New Title', content='some_random_content whey')
    ...
    def test_single_post(self):
        post_count = Lab.objects.count()
        self.assertEqual(post_count, 1)

    def test_get_list(self):
        data = {}
        url = api_reverse('api-labsoil:lab-listcreate')
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(response.data)

    def test_lab_item(self):
