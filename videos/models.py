...
class Video(models.Model):
    ...
    def get_api_url(self, request=None):
        return api_reverse("api-videos:video-rud",  kwargs={'pk': self.pk}, request=request)
    ...
    def get_share_message(self):
        full_url = "%s%s" % (settings.FULL_DOMAIN_NAME, self.get_absolute_url())
        return urllib3.quote("%s %s" % (self.share_message, full_url))
...
class Category(models.Model):
    ...
    def get_absolute_url(self):
        return reverse("videos:project_detail", kwargs={"cat_slug": self.slug})

    def get_image_url(self):
        return "%s%s" % (settings.MEDIA_URL, self.image)


TAG_CHOICES = (("compost", "compost"),
