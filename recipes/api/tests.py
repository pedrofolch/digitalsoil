...
class PileOfCompostAPITestCase(APITestCase):

    def setUp(self):
        user_obj = User.objects.create(username='testcfeuser', email='test@test.com')
        user_obj.set_password('somerandopassword')
        user_obj.save()
        pileofcompost_post = PileOfCompost.objects.create(user=user_obj, title='New Title', content='some_random_content whey')
    ...
    def test_single_post(self):
        pile_count = PileOfCompost.objects.count()
        self.assertEqual(pile_count, 1)

    def test_get_list(self):
        data = {}
        url = api_reverse('api-pileofcompost:pileofcompost-listcreate')
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(response.data)

    def test_pileofcompost_item(self):
    ...
    def test_update_item(self):
        ...
        pileofcompost_post = PileOfCompost.objects.first()
        url = pileofcompost_post.get_api_url()
