...
from assets.models import TypeOfAsset, EngineSpecs
...
class AssetAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        user_obj.save()
        asset_post = TypeOfAsset.objects.create(user=user_obj, title='New Title', content='some_random_content whey')
    ...
    def test_single_post(self):
        asset_count = TypeOfAsset.objects.count()
        self.assertEqual(asset_count, 1)

    def test_get_list(self):
        data = {}
        url = api_reverse('api-assets:asset-listcreate')
    ...
    def test_update_item(self):
        ...
        asset_post = TypeOfAsset.objects.first()
        url = asset_post.get_api_url()
...
class EngineAPITestCase(APITestCase):
    ...
    def setUp(self):
        ...
        user_obj.set_password('somerandopassword')
        user_obj.save()
        engine_post = EngineSpecs.objects.create(user=user_obj, title='New Title', content='some_random_content whey')
    ...
    def test_single_post(self):
        engine_count = EngineSpecs.objects.count()
        self.assertEqual(engine_count, 1)

    def test_get_list(self):
        data = {}
        url = api_reverse('api-assets:engine-listcreate')
        response = self.client.get(url, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        print(response.data)

    def test_sprout_item(self):
        data = {'title': 'some rando title', 'content': 'some more content'}
        url = api_reverse('api-assets:engine-listcreate')
        response = self.client.post(url, data, format='json')
        # should be 401 because unauthourized users cannot post blog
    ...
    def test_update_item(self):
        ...
        engine_post = EngineSpecs.objects.first()
        url = engine_post.get_api_url()
        data = {'title': 'some random title', 'content': 'some more content'}
        response = self.client.post(url, data, format='json')
        self.assertEqual(response.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)
